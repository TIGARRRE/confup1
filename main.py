...
def cmd_ls():
    contents = set()
    with ZipFile(vfs_path, 'r') as file:  # Open the zip file for reading
...
def cmd_rm(target):
    ...
    if not found:
        ...
        with ZipFile(vfs_path, 'w') as updated_zip:  # Open the zip file for writing (overwriting)
            for name in updated_files:
                with ZipFile(vfs_path, 'r') as file:  # Open the zip file again to read content
...
def cmd_mkdir(new_dir):
    ...
    with ZipFile(vfs_path, 'r') as file:  # Open the zip file for reading
        if path in file.namelist():
            ...
            with ZipFile(vfs_path, 'a') as file:  # Open the zip file for appending
                file.writestr(path, '')
...
def cmd_exit():
    print("Exiting virtual shell...")
    exit()

# Main loop
with ZipFile(vfs_path, 'a') as file:  # Open the zip file for appending (used only for interaction)
    while True:
